...
function computeNextRun(task: ScheduledTask, error: string | null): { nextRun: string | null; retryCount: number; error: string | null } {
  ...
  if (combinedError) {
    if (retryCount < TASK_MAX_RETRIES) {
      ...
      nextRun = new Date(Date.now() + jitter).toISOString();
    }
  } else {
    retryCount = 0;
  }
